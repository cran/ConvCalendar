...
char *month_names[13] = { "Jan", "Feb", "Mar",
...
char * DLL_FUNC set_month_name( const int month, char *new_name)

{

#ifdef OLD_WAY

   extern char *month_names[];       /* in DAT.C */
   ...
   if( new_name)
      ...
      strcpy( month_names[month - 1], new_name);
...
machines,  this will have to be fixed! */
...

{
   ...
   index = (int)year - *(short *)( chinese_calendar_data + 2);

   n_years = *(const short *)chinese_calendar_data;
